...
from twisted.internet import defer,  _sslverify
...
class HttpReqQ:
    ...
    def _processQ(self):
        if not(self.busy) and len(self.q) > 0:
            ...
            # dAdapter = treq.get('https://iam.gcdn.co/stats/put', headers=req.headers)
            dAdapter = treq.get(req.url, headers=req.headers)
            # pprint(vars(req.headers))
            ...
            timeoutCall = self.reactor.callLater(2*60, dAdapter.cancel)
...
class HlsProxy:
    ...
    def getSegmentRelativeUrl(self, item):
        ...
        # return "stream" + str(item.mediaSequence) + ".jpg"
    ...
    def onValidPlaylist(self, playlist):
        ...
        if playlist.encryption != None and not self.encryptionHandled:
            ...
            if playlist.encryption.method in ['AES-128', 'SAMPLE-AES'] and playlist.encryption.uri != '':
                ...
                key_dir = self.outDir + "key.bin"

                self.requestResource(playlist.encryption.uri, key_dir)
            else:
    ...
    def writeFile(self, filename, content):
        ...
        # if filename == 'key':
        #     print '())==D - KEY: ', filename, 'content', content
    ...
    def requestFragment(self, item):
        ...
        d = self.reqQ.request('GET', item.absoluteUrl, Headers(self.httpHeaders()), None)
        thiz = self
...
def runProxy(reactor, args):
    ...
    proxy.headers = args.H
    proxy.options = args.O
    proxy.dump_durations = args.dump_durations
    proxy.save_individual_playlists = args.save_individual_playlists

    playlist = args.hls_playlist
    if args.J :
        data_json = json.loads(args.J)
        playlist = data_json['url']

        reformat_header = {}
        for key, val in data_json['header'].iteritems():
            reformat_header[key] = [val]

        proxy.headers = json.dumps(reformat_header)
        proxy.options = data_json['options']

    pprint(vars(proxy))
    exit()
    if not(args.o is None):
        proxy.setOutDir(args.o)
    d = proxy.run(playlist)
    return d
...
def main():
    ...
    parser.add_argument("--save-individual-playlists", action=None)
    parser.add_argument("-o")
